...
{
    // Dracula: check prefix
    if(uri.scheme() != QString("Dracula"))
        return false;
...
{
    // Convert Dracula:// to Dracula:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("Dracula://"))
    {
        uri.replace(0, 12, "Dracula:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Dracula.lnk";
}
...
{
    return GetAutostartDir() / "Dracula.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Dracula-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  Dracula-qt [" + tr("command-line options") + "]                     " + "\n";
